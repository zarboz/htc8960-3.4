...
static int jet_flashlight_control(int mode)
{
...
static struct msm_camera_sensor_flash_src msm_flash_src = {
	...
	.camera_flash = jet_flashlight_control,
};
#endif
...
static int jet_rawchip_vreg_on(void)
{
	...
	/* PM8921_GPIO_PM_TO_SYS(JET_GPIO_V_RAW_1V8_EN) 1800000 */
	...
	gpio_free(JET_GPIO_RAW_1V8_EN);

	mdelay(5);

	if (system_rev >= 0 && system_rev <= 3) { /* for XA~XD */
	/* PM8921_GPIO_PM_TO_SYS(JET_V_RAW_1V2_EN) 1200000 */
...
static uint16_t msm_cam_gpio_tbl[] = {
	...
	JET_GPIO_CAM_MCLK1,
#if 0
	JET_GPIO_CAM_I2C_DAT, /*CAMIF_I2C_DATA*/
	JET_GPIO_CAM_I2C_CLK, /*CAMIF_I2C_CLK*/
#endif
	JET_GPIO_RAW_INTR0,
	JET_GPIO_RAW_INTR1,
...
static int jet_s5k3h2yx_vreg_on(void)
{
	...
	if (system_rev == 0 || !jet_use_ext_1v2()) { /* for XA || XB wo ext1v2 */
	/* digital */
...
#define JET_XB_GPIO_V_CAM2_D1V2_EN 93
static int jet_s5k6a1gx_vreg_on(void)
{
	...
	/* digital */
	if (system_rev >= 1) { /* for XB */
		rc = gpio_request(JET_XB_GPIO_V_CAM2_D1V2_EN, "s5k6a1gx");
		pr_info("[CAM]reset pin gpio_request,%d\n", JET_XB_GPIO_V_CAM2_D1V2_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", JET_XB_GPIO_V_CAM2_D1V2_EN);
			goto enable_digital_fail;
		}
		gpio_direction_output(JET_XB_GPIO_V_CAM2_D1V2_EN, 1);
		gpio_free(JET_XB_GPIO_V_CAM2_D1V2_EN);
	} else { /* for XA */
...
static int jet_s5k6a1gx_vreg_off(void)
{
	...
	if (system_rev >= 1) { /* for XB */
		...
		if (rc < 0)
			...
			gpio_direction_output(JET_XB_GPIO_V_CAM2_D1V2_EN, 0);
			gpio_free(JET_XB_GPIO_V_CAM2_D1V2_EN);
		}
	} else { /* for XA */
		rc = gpio_request(JET_GPIO_V_CAM_D1V2_EN, "s5k6a1gx");
...
static struct msm_camera_sensor_platform_info sensor_s5k6a1gx_board_info = {
	...
	.sensor_pwd	= JET_GPIO_CAM2_STANDBY,
	.vcm_pwd	= 0,
	.vcm_enable	= 0,
...
#ifdef CONFIG_MSM_CAMERA
struct i2c_board_info jet_camera_i2c_boardinfo[] = {
#ifdef CONFIG_S5K3H2YX
	{
...
struct msm_camera_board_info jet_camera_board_info = {
	.board_info = jet_camera_i2c_boardinfo,
	.num_i2c_board_info = ARRAY_SIZE(jet_camera_i2c_boardinfo),
};
#endif
