...
static void msm8960_enable_ext_spk_amp_gpio(u32 spk_amp_gpio)
{
	...
	if (spk_amp_gpio == bottom_spk_pamp_gpio) {
		...
		if (ret)
			...
			pr_info("%s: enable dock amp gpio\n", __func__);
...
static const struct soc_enum jet_msm8960_enum[] = {
...
static int jet_stereo_voice_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: jet_stereo_control = %d\n", __func__,
			jet_stereo_control);
...
static const struct snd_kcontrol_new tabla_msm8960_controls[] = {
	...
	SOC_ENUM_EXT("Stereo Selection", jet_msm8960_enum[0], jet_stereo_voice_get,
...
static int __init jet_audio_init(void)
{
	...
	pr_info("%s: register cable detect func for dock", __func__);
