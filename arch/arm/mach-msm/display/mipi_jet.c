...
static int mipi_jet_lcd_on(struct platform_device *pdev)
{
  ...
  if (!mipi_lcd_on)
    {
      mipi_dsi_cmds_tx(&jet_tx_buf, nvt_LowTemp_wrkr_enter,
                       ARRAY_SIZE(nvt_LowTemp_wrkr_enter));
      
      mipi_dsi_cmds_tx(&jet_tx_buf, nvt_LowTemp_wrkr_exit,
      ...
      if (panel_type != PANEL_ID_NONE) 
        {
          if (mipi->mode == DSI_VIDEO_MODE) 
            {
              mipi_dsi_cmds_tx(&jet_tx_buf, jet_video_on_cmds, jet_video_on_cmds_count);
              PR_DISP_INFO("%s: panel_type video mode (%d)", __func__, panel_type);
            } 
          else
            {
              mipi_dsi_cmds_tx(&jet_tx_buf, jet_command_on_cmds, jet_command_on_cmds_count);
              PR_DISP_INFO("%s: panel_type command mode (%d)", __func__, panel_type);
            }
        }
      else
        PR_DISP_INFO("%s: panel_type not supported!(%d)", __func__, panel_type);
    }
...
static int mipi_jet_lcd_off(struct platform_device *pdev)
{
  ...
  if (panel_type != PANEL_ID_NONE) {
    mipi_set_tx_power_mode(1);
    PR_DISP_INFO("%s\n", __func__);
    mipi_dsi_cmds_tx(&jet_tx_buf, jet_display_off_cmds,
...
static void mipi_jet_display_on(struct msm_fb_data_type *mfd)
{
  cmdreq.cmds = jet_display_on_cmds;
  cmdreq.cmds_cnt = jet_display_on_cmds_count;
...
static void mipi_jet_display_off(struct msm_fb_data_type *mfd)
{
  cmdreq.cmds = jet_display_off_cmds;
  cmdreq.cmds_cnt = jet_display_off_cmds_count;
  cmdreq.flags = CMD_REQ_COMMIT;
  cmdreq.rlen = 0;
  cmdreq.cb = NULL;
...
inline void mipi_dsi_set_backlight(struct msm_fb_data_type *mfd, int level)
{
  ...
  cmdreq.cmds = jet_cmd_backlight_cmds;
  cmdreq.cmds_cnt = jet_cmd_backlight_cmds_count;
  cmdreq.flags = CMD_REQ_COMMIT;
  cmdreq.rlen = 0;
  cmdreq.cb = NULL;
...
static struct msm_fb_panel_data jet_panel_data = {
  ...
  .display_on = mipi_jet_display_on,
  .display_off = mipi_jet_display_off,
#ifdef CONFIG_FB_MSM_CABC
